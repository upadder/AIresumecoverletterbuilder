...
def heading(name, email, phone, linkedin, github):
    ...
    \\textbf{{\\LARGE {'''+str(name)+'''}}} & Email : \\href{mailto:'''+str(email)+'''}{'''+str(email)+'''}\\\\
    \\href{'''+str(linkedin)+'''}{Linkedin: 
    '''+str(linkedin)+'''} & Mobile : 
    '''+str(phone)+''' \\\\
    \\href{
    '''+str(github)+'''}{Github:
...
def education(university, location, degree, gpa, time_period, relevent_courses):
    return '''
        \\resumeSubheading
        {'''+str(university)+'''}{'''+str(location)+'''}
        {'''+str(degree)+''';  GPA: '''+str(gpa)+'''}{'''+str(time_period)+'''}
        
        {\\scriptsize \\textit{Courses: '''+str(relevent_courses)+'''.}}
        '''
